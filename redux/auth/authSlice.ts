import { IAuthData, IAuthInitState, ILoginData, ISignUpData } from '@/types';
	...
	async (SignUpData, { rejectWithValue }) => {
		try {
			...
			const response = await axios.post(
				`https://nextjs-ecommerce-backend.vercel.app/api/v1/auth/signup`,
				SignUpData
			);
...
export const loginUser = createAsyncThunk<IAuthData, ILoginData>(
	...
	async (LoginData, { rejectWithValue }) => {
		try {
			const response = await axios.post(
				`https://nextjs-ecommerce-backend.vercel.app/api/v1/auth/login`,
				LoginData
			);
